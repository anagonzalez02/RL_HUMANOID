...
import humanoid_with_bridge_env  # se registra al importarse
...
if __name__ == "__main__":
    ...
    videos_dir = os.path.join(current_dir, "videosPuente", folder_name)
    os.makedirs(videos_dir, exist_ok=True)
    checkpoint_dir = os.path.join(current_dir, "checkpoints", folder_name)
    os.makedirs(checkpoint_dir, exist_ok=True)

    # Create the tensorboard writer
    log_dir = os.path.join(current_dir, "logsPuente", folder_name)
    ...
    test_env=RecordVideo(
        ...
        name_prefix="Puente1",
        episode_trigger=lambda episode_id: episode_id % N == 0,
        )
    slopes = [0.05, 0.10, 0.15, 0.174]
    epochs_per_stage = args.n_epochs // 4
    k_progs = [50.0, 100.0, 150.0, 200.0]
    ...
    episode_lengths = []                                    # guarda la longitud de cada episodio
    step_counts = np.zeros(args.n_envs, dtype=int)
    try:
        for epoch in range(1, args.n_epochs + 1):
            # Determina en qué “nivel” estamos
            stage = min((epoch-1) // epochs_per_stage, len(slopes)-1)
            current_slope = slopes[stage]

            # Actualiza la pendiente en todos los envs vectorizados
            envs.set_attr('slope_angle_up',   current_slope)
            envs.set_attr('slope_angle_down', -current_slope)

            # Y también en test_env si quieres que el vídeo refleje la misma pendiente
            test_env.unwrapped.slope_angle_up   = current_slope
            test_env.unwrapped.slope_angle_down = -current_slope

            current_k_prog = k_progs[stage]
            envs.set_attr("k_prog", current_k_prog)
            test_env.unwrapped.k_prog = current_k_prog


            if (epoch-1) % epochs_per_stage == 0:
                print(f"=== Curriculum: etapa {stage+1}/{len(slopes)}, pendiente = {current_slope:.3f} rad ===")
            ...
            for _ in tqdm(range(0, args.n_steps), desc=f"Epoch {epoch}: Collecting trajectories"):
                ...
                next_obs, rewards, next_terminateds, next_truncateds, infos = envs.step(actions.cpu().numpy())

                    # ——— actualizar longitud de episodio y reinicios ———
                step_counts += 1
                dones = next_terminateds.astype(bool) | next_truncateds.astype(bool)
            ...
            if epoch % args.render_epoch == 0:
                ...
                #1) LOG: longitud media
                    # longitud media de episodio esta época
                mean_length = float(np.mean(episode_lengths)) if episode_lengths else 0.0
                writer.add_scalar("episode/length_mean", mean_length, epoch)
                # opcional: tiempo de época
                epoch_time = time.time() - start_time
