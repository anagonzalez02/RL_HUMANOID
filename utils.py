...
def parse_args_ppo() -> argparse.Namespace:
    ...
    parser.add_argument("--cuda", default=True if torch.cuda.is_available() else False, action="store_true",
                        help="Use CUDA")
    parser.add_argument("--env", default="HumanoidOnBridge-v0", help="Environment to use")
...
def log_video(env, agent, device, video_path, fps=30):
    ...
    while not done:
        ...
        # Usando el nombre de la cámara definido en el XML
        #frames.append(env.render(mode="rgb_array", camera_name="overhead"))
        # Render directo con el simulador de MuJoCo, usando la cámara "overhead"
        #raw = env.unwrapped
        # 640×480 es un tamaño estándar; ajústalo si necesitas otra resolución
        #frame = env.unwrapped.render(camera_name="overhead")
        #frames.append(frame)
